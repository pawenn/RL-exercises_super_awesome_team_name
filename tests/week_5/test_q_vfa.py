...
import numpy as np
import torch
from rl_exercises.week_5.q_learning_vfa import EpsilonGreedyPolicy, VFAQAgent
...
class TestQLearning(unittest.TestCase):
    def test_make_Q(self):
        ...
        v = q.predict_action(np.array([state]), info)
    ...
    def test_vfa_update(self):
        ...
        policy = partial(EpsilonGreedyPolicy, epsilon=0.0)
        q = VFAQAgent(env, policy, 0.1, 0.99)
        loss = q.update_agent([states, actions, rewards, next_states, dones, infos])
    ...
    def test_deterministic_case1(self):
        ...
        self.assertAlmostEqual(q.b.sum().item(), -1.1226010322, places=5)
        self.assertAlmostEqual(q.W.sum().item(), -2.3177227973, places=5)
